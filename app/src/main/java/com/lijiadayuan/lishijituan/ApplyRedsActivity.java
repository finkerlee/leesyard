...
public class ApplyRedsActivity extends BaseActivity implements OnClickListener {
    //视图bean
    private Reds mReds;
    ...
    private static ApplyRedsActivity instance;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mReds = getIntent().getParcelableExtra(KeyConstants.IntentPageValues.Reds);
        if ("".equals(mReds.getRedId())){
            Toast.makeText(ApplyRedsActivity.this,"获取红包信息失败",Toast.LENGTH_LONG).show();
            return;
        }
        initView();
        setListener();
        ...
        mGridView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                if (i == 4) {
                    ...
                    dialog = new ReceiveDialog(ApplyRedsActivity.this, R.style.protocol_dialog);
...
//        });
    ...
    public static ApplyRedsActivity getInstance() {
        return instance;
    }
        ...
        switch (v.getId()) {
            ...
            case R.id.btn_receive:
                ...
                if (mEtName.getText().toString().isEmpty()){
                    Toast.makeText(ApplyRedsActivity.this,"请填写申请人姓名",Toast.LENGTH_LONG).show();
                    return;
                }
                if (mEtAccount.getText().toString().isEmpty()){

                    Toast.makeText(ApplyRedsActivity.this,"请添加开户行名称",Toast.LENGTH_LONG).show();
                    return;
                }
                if (mEtPhoneNum.getText().toString().isEmpty()){
                    Toast.makeText(ApplyRedsActivity.this,"请填写手机号",Toast.LENGTH_LONG).show();
                    return;
                }
                if (mBitmaps.size() == 1){
                    Toast.makeText(ApplyRedsActivity.this,"请上传图片",Toast.LENGTH_LONG).show();
                    return;
                }
                ...
                UpLoadImageTask1 mUpLoadImageTask1 = (UpLoadImageTask1) new UpLoadImageTask1(ApplyRedsActivity.this,bitmaps).execute(UrlConstants.RED_UPLOAD);
                mUpLoadImageTask1.setCallBACK(new UpLoadPicCallBack() {
                    ...
                    public void setCompleteImage(final ArrayList<String> iamgePicList) {
                        if (iamgePicList.size()>0){
                            ...
                            StringRequest mStringRequest = new StringRequest(Request.Method.POST, UrlConstants.RED_APPLY, new Response.Listener<String>() {
                                @Override
                                public void onResponse(String response) {
                                    ...
                                    if (JsonParseUtil.isSuccess(mJsonObj)){
                                        ...
                                        if ("1".equals(response_data)){
                                            Toast.makeText(ApplyRedsActivity.this,"提交成功，请耐心等待",Toast.LENGTH_LONG).show();
                                            finish();
                                        }else{
                                            Toast.makeText(ApplyRedsActivity.this,response_data,Toast.LENGTH_LONG).show();
                                        }
                                    }
                                }
                                ...
                                public void onErrorResponse(VolleyError error) {
                                    ...
                                    Toast.makeText(ApplyRedsActivity.this,"faile",Toast.LENGTH_LONG).show();
                                }
                            }){
                                @Override
                                protected Map<String, String> getParams() throws AuthFailureError {
                                    Map<String, String> params = new HashMap<String, String>();
                                    params.put("redId", mReds.getRedId());//红包id
                                    params.put("userId",UsersUtil.getUserId(ApplyRedsActivity.this));//用户id
                                    ...
                                    if (iamgePicList.size()>0){
                                        for (int i = 0;i<iamgePicList.size();i++){
                                            params.put("raImg"+(i+1),iamgePicList.get(i));
                                        }
                                    }
                                    return params;
                                }
